...
class Lox:
    ...
    def run(self, source: str) -> None:
        ...
        if statements == [None]:
            self.missingSemicolonError(tokens)
        if self.hadError:
            return
        if self.hadRuntimeError:
            return
    ...
    def missingSemicolonError(self, tokens: list[Token.Token]) -> None:
        lastToken = tokens[-2]
        if (lastToken.type == TokenType.STRING):
            return self.tokenError(lastToken, "Missing semicolon :/")
